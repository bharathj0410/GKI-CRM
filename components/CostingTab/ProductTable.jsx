...
export default function ProductTable({ row, handleSelectionChange, data }) {
  ...
  const layer =
    String(row.layers).toLocaleLowerCase().replaceAll(" ", "") + "Type";

  useEffect(() => {
    handleSelectionChange(
      layer,
      String(row.layers).includes("Flute") ? "Flute" : "Liner",
    );
  }, [row.layers]);
  useEffect(() => {
    if (!data[layer]) {
  ...
  return (
    <div className="w-full 0px 1px 4px; rounded-2xl mt-5">
      ...
      <div className="grid grid-cols-4 gap-3">
        ...
        <div className="w-full">
          <div className="rounded text-xs  text-zinc-500 w-full">{row?.bf}</div>
        </div>
        <div className="w-full">
          <div className="rounded text-xs  text-zinc-500 w-full">
            {row?.gsm}
        ...
        <div className="w-full">
          <div className="rounded text-xs  text-zinc-500 w-full">{row?.bs}</div>
        </div>
        <div className="w-full">
          <div className="rounded text-xs  text-zinc-500 w-full">
            {row?.cobb}
