          ...
          {isEditing ? (
            <Input
              ...
              classNames={{
                input: "bg-white",
                inputWrapper:
                  "border-gray-200 hover:border-gray-300 focus-within:border-secondary",
              }}
            />
          ) : (
          ...
          {isEditing ? (
            ...
            <p className="text-gray-900 font-semibold">
              {editedEmployee.gender
                ? editedEmployee.gender.charAt(0).toUpperCase() +
                  editedEmployee.gender.slice(1)
                : "N/A"}
            </p>
          )}
        </div>
                ...
                {(editedEmployee.languagesKnown || []).map((lang) => (
                  <Chip
                    ...
                    onClose={() => {
                      handleInputChange(
                        "languagesKnown",
                        (editedEmployee.languagesKnown || []).filter(
                          (l) => l !== lang,
                        ),
                      );
                    }}
              ...
              >
                {languagesData.map((lang) => (
                  <SelectItem
                    key={lang.key}
                    value={lang.key}
                    isDisabled={(editedEmployee.languagesKnown || []).includes(
                      lang.key,
                    )}
                  >
                    {lang.label}
            ...
            <div className="flex flex-wrap gap-2">
              {(employee.languagesKnown || []).length > 0 ? (
                (employee.languagesKnown || []).map((lang) => (
                  <Chip key={lang} color="secondary" variant="flat">
                    {languagesData.find((l) => l.key === lang)?.label || lang}
                  </Chip>
                ))
              ) : (
