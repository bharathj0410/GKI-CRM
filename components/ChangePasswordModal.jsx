...
export default function ChangePasswordModal({ isOpen, onClose, employee, onSuccess }) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            Toast("Error", err.response?.data?.error || "Failed to change password", "danger");
        } finally {
            setLoading(false);
        }
    };
            ...
            <ModalContent>
                <ModalHeader className="flex flex-col gap-1">
                    ...
                    <p className="text-sm text-gray-500 font-normal">
                        Change password for <span className="font-semibold text-secondary">{employee?.name}</span>
                    </p>
                </ModalHeader>
                <ModalBody>
