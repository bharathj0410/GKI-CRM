...
export default function TagInput({ tableName, hasPrice }) {
  ...
  useEffect(() => {
    axios
      .get(`getConfigData?tableName=${tableName}&hasPrice=${hasPrice}`)
      .then((data) => {
    ...
    if (e.key === "Enter") {
      ...
      const isDuplicate = tags.some(
        (tag) =>
          (typeof tag === "string"
            ? tag.toLowerCase()
            : tag.key.toLowerCase()) === trimmed.toLowerCase(),
      );
  ...
  const normalize = (arr) =>
    arr
      .map((tag) => (typeof tag === "string" ? { key: tag, price: 0 } : tag))
      .sort((a, b) => a.key.localeCompare(b.key));

  const saveData = (tagsToSave = tags) => {
    axios
      .post("/addConfigData", { key: tableName, value: tagsToSave, hasPrice })
      .then((response) => {
                ...
                <Input
                  ...
                  startContent={
                    <div className="pointer-events-none flex items-center">
                      <span className="text-default-500 text-base font-semibold">
                        ₹
                      </span>
                    </div>
                  }
