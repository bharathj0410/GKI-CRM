import { Avatar, Chip, Card, CardBody, Dropdown, DropdownTrigger, DropdownMenu, DropdownItem, Button } from '@heroui/react'
import Image from 'next/image'
import React from 'react'
import { UserIcon, EnvelopeIcon, PhoneIcon, ShieldCheckIcon, KeyIcon, EllipsisVerticalIcon, PencilIcon, TrashIcon, LockClosedIcon, EyeIcon } from '@heroicons/react/24/outline'
import { useAuth } from '@/contexts/AuthContext'

export default function EmployeeCard({ Employee, onEdit, onDelete, onChangePassword, onClick }) {
        ...
        >
            <CardBody className='p-4'>
                ...
                <div className='flex flex-col items-center cursor-pointer' onClick={onClick}>
                    ...
                    {/* Name and Username */}
                    <h3 className='text-base font-bold text-gray-800 mb-0.5 text-center'>{Employee?.name || 'N/A'}</h3>
                    <div className='flex items-center gap-1 mb-3'>
                        <UserIcon className='w-3.5 h-3.5 text-gray-500' />
                        <p className='text-xs text-gray-600'>@{Employee?.username || 'N/A'}</p>
                    </div>
                    ...
                    <div className='w-full space-y-2'>
                        ...
                        {Employee?.email && (
                            <div className='flex items-center gap-2 p-1.5 rounded-lg hover:bg-purple-50 transition-colors'>
                                ...
                                <span className='text-xs text-gray-700 truncate'>{Employee.email}</span>
                            </div>
                        )}
                        ...
                        {Employee?.phone && (
                            <div className='flex items-center gap-2 p-1.5 rounded-lg hover:bg-purple-50 transition-colors'>
                                ...
                                <span className='text-xs text-gray-700'>+91 {Employee.phone}</span>
                            </div>
                        )}
                        ...
                        {Employee?.permissions && Employee.permissions.length > 0 && (
                            <div className='mt-2'>
                                <div className='flex items-center gap-1.5 mb-1.5'>
                                    ...
                                    <span className='text-[10px] font-semibold text-gray-600 uppercase'>Access</span>
                                </div>
