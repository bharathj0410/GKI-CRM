...
export default function QuotationContent() {
  ...
  useEffect(() => {
    if (productId && orderId) {
      axios
        .get(`/generateQutation?productId=${productId}&orderId=${orderId}`)
        .then((data) => {
          setData(data.data);
    ...
    <div className="flex items-center justify-center gap-[5rem]">
      <div className="w-[45rem]">
        <div className="w-full text-xs relative" ref={ref}>
          <img
            src="watermark.svg"
            alt="watermark"
            className="w-[20rem] text-zinc-100 absolute bottom-0 left-0"
          />
          <img
            src="watermark.svg"
            alt="watermark"
            className="w-[20rem] text-zinc-100 absolute top-[10rem] right-0"
          />
          <div className="z-20 relative">
            <div className="w-full items-center flex justify-center relative ">
              <div className="flex flex-col items-center justify-center pl-7">
                ...
                <div>
                  #34/D, Industrial ‘A’ Layout, Bannimantap, Mysore -570015
                </div>
                <div className="flex gap-10 pt-5">
                  <div>Phone : 9448067061</div>
                  <div>GSTIN : 29ALF3687R1ZV</div>
                </div>
              </div>
            </div>
            ...
            <div className="pt-[2rem]">
              ...
              <div className="pl-[2rem]">
                ...
                <div className="w-[25rem]">
                  {data["order_data"]["billing_address_line"]}
                </div>
                <div>
                  {data["order_data"]["billing_city"]},{" "}
                  {data["order_data"]["billing_state"]},{" "}
                  {data["order_data"]["billing_country"]}
                </div>
              </div>
            </div>
            ...
            <div className="pt-[3rem]">
              <div>Dear Sir,</div>
              <div className="pl-[2rem]">
                As per the samples provided, we are pleased to submit our
            ...
            <div className="w-full rounded-2xl py-5 ">
              <div className="grid grid-cols-[40px_50px_180px_130px_80px_90px_70px_70px] w-full bg-zinc-100 rounded-xl text-center">
                {data["table_name"].map((heading) => (
                  <div
                    key={heading}
                    className="py-2 px-2 text-xs font-semibold text-zinc-500"
                  >
                    {heading}
                  </div>
                ))}
              ...
              {formArray.map((productData, idx) => (
                ...
                >
                  {Object.entries(productData).map(([key, value]) => (
                    <div
                      key={key}
                      className="py-2 px-2 text-xs font-semibold text-zinc-700"
                    >
                      {value}
                    </div>
                  ))}
              ...
              <div className="pt-[2rem]">
                <div className="text-center">
                  We assure you of the best quality and service. We look forward
                  to receiving your valuable orders.
                </div>
                <div className="pt-[2rem]">
                  <div>Terms & Conditions:</div>
                  <ul className="pl-[2rem]">
                    <li>
                      <strong>Taxes:</strong> 18% GST applicable on sales
                    </li>
                    <li>
                      <strong>Transportation:</strong> Charges extra
                    </li>
                    <li>
                      <strong>Payment:</strong> 100% in advance
                    </li>
                    <li>
                      <strong>Additional Charges:</strong> Development charges
                      extra as applicable
                    </li>
                    <li>
                      <strong>Delivery Schedule:</strong> Within 20 days from
                      the date of Purchase Order (PO)
                    </li>
                  </ul>
              ...
              <div className="pt-[8rem] text-right pr-[2rem]">
                Founder / CEO
              </div>
            </div>
          </div>
      ...
      <div>
          ...
          <div key={productData["sl. No"]} className="py-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Input
                ...
                onChange={(e) => handleChange(e, productData["sl. No"])}
              />
              <Input
                label="Size"
                type="text"
              ...
              <Input
                ...
                value={productData["Printing charges"]}
                onChange={(e) => handleChange(e, productData["sl. No"])}
              />
              <Input
                ...
                type="text"
                name="Price per unit"
                value={productData["Price per unit"]}
                onChange={(e) => handleChange(e, productData["sl. No"])}
              />
            </div>
          </div>
        ...
        <div className="py-10">
          ...
          <Button color="secondary" onPress={handleSaveQuotation}>
            Save
          </Button>
        </div>
      </div>
