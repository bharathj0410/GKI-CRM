...
import { Card, CardBody, Button, Divider, Chip, Tabs, Tab, Modal, ModalContent, ModalHeader, ModalBody, ModalFooter, useDisclosure } from "@heroui/react";
import { ArrowLeftIcon, DocumentCurrencyRupeeIcon, PencilIcon, TrashIcon, PlusIcon, InboxArrowDownIcon, CubeIcon } from '@heroicons/react/24/outline';
...
export default function ProductDetailPage({ params }) {
  ...
  useEffect(() => {
    if (productId) {
      ...
      axios.get(`getOrderDetails?parentId=${id}`).then((response) => {
        ...
        if (product?.id) {
          axios.get(`getOrderDetails?parentId=${product.id}`).then((res) => {
      ...
      // Fetch quotations
      axios.get(`getQuotation?id=${id}`).then((response) => {
        ...
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">View and manage product information</p>
            </div>
          </div>
        </div>
          ...
          <div className="p-6">
            ...
            <div className="flex items-center gap-4 mb-5">
              ...
              <div className="flex-1 min-w-0">
                ...
                {productData.enquiryDate && (
                  <p className="text-sm text-gray-400 dark:text-gray-500 mt-0.5">Enquiry: {productData.enquiryDate}</p>
                )}
              </div>
              <Chip
                variant="flat"
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-x-8 gap-y-4 mb-6">
              <div>
                ...
                <p className="text-base font-semibold text-gray-800 dark:text-gray-200">{productData.boxType || "—"}</p>
              </div>
              <div>
                <p className="text-[11px] text-gray-400 dark:text-gray-500 font-medium leading-none mb-1">Order Quantity</p>
                <p className="text-base font-semibold text-gray-800 dark:text-gray-200">{productData.orderQuantity || "—"}</p>
              </div>
              <div>
                <p className="text-[11px] text-gray-400 dark:text-gray-500 font-medium leading-none mb-1">Printing Type</p>
                <p className="text-base font-semibold text-gray-800 dark:text-gray-200">{productData.printingType || "—"}</p>
              </div>
              <div>
                <p className="text-[11px] text-gray-400 dark:text-gray-500 font-medium leading-none mb-1">Color Type</p>
                <p className="text-base font-semibold text-gray-800 dark:text-gray-200">{productData.colorType || "—"}</p>
              </div>
              <div>
                <p className="text-[11px] text-gray-400 dark:text-gray-500 font-medium leading-none mb-1">Flap Join Type</p>
                <p className="text-base font-semibold text-gray-800 dark:text-gray-200">{productData.flapJointType || "—"}</p>
              </div>
              <div>
                <p className="text-[11px] text-gray-400 dark:text-gray-500 font-medium leading-none mb-1">Pieces Per Bundle</p>
                <p className="text-base font-semibold text-gray-800 dark:text-gray-200">{productData.piecesPerBundle || "—"}</p>
              </div>
            </div>
            ...
            {(onClose) => (
              <>
                <ModalHeader className="flex flex-col gap-1">
                  {modalType === 'edit' ? 'Edit Product' : modalType === 'addSubProduct' ? 'Add Sub Product' : 'Delete Product'}
                </ModalHeader>
                <ModalBody>
        ...
        <div className="mt-6">
          <Tabs 
            ...
            classNames={{
              ...
              tabContent: "group-data-[selected=true]:text-secondary-600 dark:group-data-[selected=true]:text-secondary-400"
            }}
          >
            {/* Sub Products Tab */}
            <Tab key="sub-products" title="Sub Products">
              <div className="mt-4">
                <div className="flex justify-between items-center mb-5">
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white">Sub Products</h3>
                  <Button 
                    color="secondary" 
                    size="sm"
                ...
                {subProducts.length > 0 ? (
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                    {subProducts.map((subProduct) => {
                      const subProductId = subProduct.id.includes('/') ? subProduct.id.split('/').pop() : subProduct.id;
                      return (
                      <div
                        key={subProduct.id}
                        onClick={() => router.push(`/costing/customer/${id}/${productId}/${subProductId}`)}
                      ...
                      >
                        ...
                        <div className="p-5">
                          ...
                          <div className="flex items-center gap-3 mb-4">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {subProduct.enquiryDate && (
                                <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{subProduct.enquiryDate}</p>
                              )}
                            </div>
                          ...
                          <div className="grid grid-cols-2 gap-x-5 gap-y-2.5">
                            <div>
                              <p className="text-[11px] text-gray-400 dark:text-gray-500 font-medium leading-none mb-0.5">Box Type</p>
                              <p className="text-sm font-semibold text-gray-800 dark:text-gray-200 truncate">{subProduct.boxType || "—"}</p>
                            </div>
                            <div>
                              <p className="text-[11px] text-gray-400 dark:text-gray-500 font-medium leading-none mb-0.5">Order Qty</p>
                              <p className="text-sm font-semibold text-gray-800 dark:text-gray-200 truncate">{subProduct.orderQuantity || "—"}</p>
                            </div>
                            <div>
                              <p className="text-[11px] text-gray-400 dark:text-gray-500 font-medium leading-none mb-0.5">Printing</p>
                              <p className="text-sm font-semibold text-gray-800 dark:text-gray-200 truncate">{subProduct.printingType || "—"}</p>
                            </div>
                            <div>
                              <p className="text-[11px] text-gray-400 dark:text-gray-500 font-medium leading-none mb-0.5">Color Type</p>
                              <p className="text-sm font-semibold text-gray-800 dark:text-gray-200 truncate">{subProduct.colorType || "—"}</p>
                            </div>
                          </div>
                          {/* Footer */}
                  ...
                  <div className="flex flex-col items-center justify-center py-16 rounded-xl bg-white dark:bg-gray-800 border border-gray-200/80 dark:border-gray-700">
                    ...
                    <p className="text-gray-600 dark:text-gray-400 font-semibold">No sub products available</p>
                    <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">Add sub products to see them here</p>
                  </div>
                )}
              </div>
            </Tab>
              ...
              <div className="mt-4">
                <div className="flex justify-between items-center mb-5">
                  ...
                  <Button 
                    color="secondary"
                    size="sm"
                    startContent={<DocumentCurrencyRupeeIcon className='w-4 h-4' />}
                ...
                {quotations && quotations.length > 0 ? (
                  <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
                    {quotations.map((data) => <QuotationCard data={data} key={data.id} />)}
                  </div>
                ) : (
                  <div className="flex flex-col items-center justify-center py-16 rounded-xl bg-white dark:bg-gray-800 border border-gray-200/80 dark:border-gray-700">
                    <div className="p-4 rounded-2xl bg-gray-100 dark:bg-gray-700/50 mb-4">
                    ...
                    <p className="text-gray-600 dark:text-gray-400 font-semibold">No quotations available</p>
                    <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">Generate a quotation to see it here</p>
                  </div>
                )}
              </div>
            </Tab>
            ...
            <Tab key="jobs" title="Jobs">
              <div className="mt-4">
                <div className="flex flex-col items-center justify-center py-16 rounded-xl bg-white dark:bg-gray-800 border border-gray-200/80 dark:border-gray-700">
                  ...
                  <p className="text-gray-600 dark:text-gray-400 font-semibold">Jobs section coming soon</p>
                  <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">Job management features will be available here</p>
                </div>
              </div>
            </Tab>
          </Tabs>
