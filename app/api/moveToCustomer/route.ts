  ...
  try {
    ...
    if (record.type === "customer") {
      return NextResponse.json(
        { error: "This record is already a customer" },
        { status: 400 },
      );
    }
    ...
    await db
      ...
      .updateOne({ _id: record._id }, { $set: { type: "customer" } });

    return NextResponse.json({
      message: "Guest successfully moved to Customer!",
    });
  } catch (err) {
    console.error("Error moving guest to customer:", err);

    return NextResponse.json(
      { error: "Failed to move guest to customer" },
      { status: 500 },
    );
  }
}
