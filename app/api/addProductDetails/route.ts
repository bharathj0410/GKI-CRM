...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.id) {
      ...
      console.log(id)
      const result = await db.collection("orderData").findOneAndUpdate(
        { id },
        { $set: updateFields },
      ...
      return NextResponse.json({ message: "Product details updated!", product: result.value })
    } else {
      // Efficiently insert new product details
      const lastProduct = await db.collection("orderData")
      ...
      const newId = lastProduct.length
        ...
        : "GKI/ID-AA000001"
      const insertResult = await db.collection("orderData").insertOne({ ...body, id: newId })
      if (!insertResult.acknowledged) {
        return NextResponse.json({ error: "Insert failed" }, { status: 500 })
      }
      return NextResponse.json({ message: "The BillInfo was successfully added!", id: newId })
    }
  } catch (err: any) {
