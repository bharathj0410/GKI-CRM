...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    let data;
    if (body.id) {
      data = await db.collection("guestData").findOneAndUpdate({ id: body.id },
        { age: 30 },
        { upsert: true })
    } else {
      ...
      if (existingdata.length) {
        ...
        data = await db.collection("guestData").insertOne({ ...body, "id": "GKI/ID-AA000001" })
      }
      if (!data.acknowledged) {
        return NextResponse.json({ error: "Insert failed" }, { status: 500 });
      }
      const guestInfo = await db.collection("guestData").findOne({ _id: data.insertedId });
      return NextResponse.json({ message: "Guest saved successfully!", id: guestInfo?.id });
    }
