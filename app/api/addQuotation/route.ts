...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (existingdata.length) {
      ...
      data = await db.collection("quotations").insertOne({ ...body, "id": "GKI/ID-AA000001" })
    }
    if (!data.acknowledged) {
      return NextResponse.json({ error: "Insert failed" }, { status: 500 });
    }
    const billInfo = await db.collection("quotations").findOne({ _id: data.insertedId });
    return NextResponse.json({ message: "The Quotations was successfully added !", id: billInfo?.id });
  }
