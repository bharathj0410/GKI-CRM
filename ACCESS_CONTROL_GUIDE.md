...
```javascript
{
  canEdit && <Button onPress={handleEdit}>Edit</Button>;
}
...
export default function EmployeeCard({ Employee, onEdit, onDelete, onClick }) {
  ...
  return (
    <DropdownMenu>
      {canView && <DropdownItem onPress={onClick}>View Details</DropdownItem>}
      {canEdit && (
...
{
  canDelete && <Button color="danger">Delete</Button>;
}

// In documents section
{
  canEdit && (
    <Button onPress={() => setShowDocModal(true)}>Add Document</Button>
  );
}
